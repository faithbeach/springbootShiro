...
public class UserServiceImpl implements UserService {
	...
	public JSONObject addUser(JSONObject jsonObject) {
		...
		if(	jsonObject.getIntValue("roleId")==6||
			jsonObject.getIntValue("roleId")==7){
			JSONObject insert =new JSONObject();
			insert.put("deleteStatus",DeleteStatus.NORMAL.getCode());//插入删除状态
			insert.put("userId",jsonObject.getIntValue("userId"));//插入userId
			Long creatorId = CommonUtil.getUserId();
			insert.put("createdBy",creatorId);//插入创建者id
			if(jsonObject.getIntValue("roleId")==6)
				businessDao.addBusiness(insert);
			else customerDao.addCustomerDao(insert);
		}
