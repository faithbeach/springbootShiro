...
public class CommonUtil {
	...
	/**
	 * @author: lwj
	 * @description: 返回当前使用用户在sys_user表里的 name
	...
	/**
	 * 将JSONObject里所有之指定的String类型的value去除两边空格
	 * */
	public static void getAllStringTrimed(JSONObject jsonObject, String requiredColumns){
		if (!StringTools.isNullOrEmpty(requiredColumns)) {
			//验证字段非空
			String[] columns = requiredColumns.split(",");
			for (String column : columns) {
				String val = jsonObject.getString(column.trim());
				jsonObject.put(column.trim(),val.trim());
			}
		}
	}

	/**
	 * 验证是否含有全部必填字段,只针对String类型的value
	 *
	 * @param requiredColumns 必填的参数字段名称 逗号隔开 比如"userId,name,telephone"
	 */
	public static void hasAllRequiredString(final JSONObject jsonObject, String requiredColumns) {
		if (!StringTools.isNullOrEmpty(requiredColumns)) {
			//验证字段非空
			String[] columns = requiredColumns.split(",");
			String missCol = "";
